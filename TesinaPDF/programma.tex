...
\end{equation}
Dato che utilizzer\`o una forma gaussiana per il pacchetto, nel programma non far\`o mai l'integrale in quanto la trasformata di Fourier di una gaussiana \`e una gaussiana.
Ovvero avr\`o:
\begin{equation}
...
\begin{equation}
\Psi(x,t) = Ae^{-\frac{\lrt{x-x_0-\o t}^2}{2\sigma}}e^{ik\lrt{x-\o t}}
\end{equation}
...
I test sull'errore li ho effettuati utilizzando come condizione iniziale un pacchetto d'onda gaussiano. Per effetto della dispersione \`e un pacchetto deltiforme che nel tempo si \`e disperso allargandosi in una gaussiana.
...
\begin{figure}[htb]
  ...
  \begin{subfigure}[b]{0.49\textwidth}
    ...
    \caption{larghezza 2}
  \end{subfigure}
  \caption{L'evoluzione del ``bump'', le ``increspature'' sull'onda che si vedono ai lati sono le interferenze dovute alla riflessione}
\end{figure}
...
Ho fatto diversi fit, sul parametro $\sigma$, con diverse masse e diversi valori di $\sigma$ iniziali, utilizzando come funzione da fittare \eqref{eq:sigmat}. L'algoritmo sembra rispettare la dispersione,  come si pu\`o vede re in \autoref{fig:dispersioneFitSigma1}, in
