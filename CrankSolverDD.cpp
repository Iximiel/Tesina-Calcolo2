...
CranckSolver::CranckSolver(const tridiag &mat, int Ns, int Nt,/* std::string opt,*/ double CCi, double CCe)
  :CS_mat(mat){
  /*  options = opt;
  if(options == "DD")
    CCfstep= CCestep = 1;
...
//0 e` l'indice delle condizioni al contorno, se uso Dirichlet 0 serve a non aggiungere eccezioni all'algoritmo
  ...
  //if(options ==" DD")
    p[0] = 0;
    /*else{
      p[0] = CS_mat.pi(0,0,CS_mat.bi(0,0,
      CS_data[tm1],CS_data[tm1+1]););
      }*/
